---
title: "Vita Green Scan Sales Report (MNG and WTC)"
...
temp_data <- rbind(
  raw_data_6 %>%
    group_by(Brand, ITEMNAME_CHI, YEAR, MONTH) %>%
    summarise(PROD_REV = sum(PROD_REV)
              ,PROD_QTY = sum(PROD_QTY)) %>%
    mutate(CHANNEL = "Total: MNG + WTC")
  ,
  raw_data_6 %>%
    group_by(CHANNEL, Brand, ITEMNAME_CHI, YEAR, MONTH) %>%
    summarise(PROD_REV = sum(PROD_REV)
              ,PROD_QTY = sum(PROD_QTY))
) %>%
  mutate(CHANNEL = factor(CHANNEL, levels = c("Total: MNG + WTC", "MNG", "WTC"))
         ,ITEMNAME_CHI = factor(ITEMNAME_CHI)) %>%
  mutate(MONTH_ABBR = factor(month.abb[MONTH],levels=month.abb)) %>%
  filter(!Brand %in% c("DC Combo", "INMED", "VCM"))
temp_data$Brand <- droplevels(temp_data$Brand)

temp_data <- temp_data %>%
  group_by(CHANNEL, Brand, ITEMNAME_CHI, YEAR) %>%
  summarise(PROD_REV = sum(PROD_REV)
            ,PROD_QTY = sum(PROD_QTY)) %>%
  ...
  mutate(ITEMNAME_CHI = factor(ITEMNAME_CHI, levels = unique(ITEMNAME_CHI)))

sankey_node <- data.frame(node_label = c(levels(temp_data$Brand), levels(temp_data$ITEMNAME_CHI))) %>%
...
sankey_data <- temp_data %>% left_join(sankey_node, by = c("Brand" = "node_label")) %>% rename("link_source" = node_label_index) %>%
  left_join(sankey_node, by = c("ITEMNAME_CHI" = "node_label")) %>% rename("link_target" = node_label_index) %>%
...
#   temp_data <- temp_data %>%
#     # YOY
#     group_by(CHANNEL, Brand, ITEMNAME_CHI, MONTH) %>%
#     mutate(LAST_YEAR = ifelse((YEAR - lag(YEAR, 1)) > 1, NA, lag(YEAR, 1))
#            ,measure_value_LAST_YEAR = ifelse((YEAR - lag(YEAR, 1)) > 1, NA, lag(get(measure_value), 1))
#     ) %>%
#     mutate(YOY = get(measure_value) - measure_value_LAST_YEAR
...
#     ) %>%
#     # MOM
#     group_by(CHANNEL, Brand, ITEMNAME_CHI) %>%
#     mutate(measure_value_LAST_MONTH =  lag(get(measure_value), 1)
#            ,MOM = get(measure_value) - lag(get(measure_value), 1)
#            ,MOM_PCT = get(measure_value)/lag(get(measure_value), 1)-1
#     )
...
#     # Order ITEMNAME_CHI by YEAR, desc(PROD_REV), filtering YEAR will affect the order
#     ungroup() %>%
#     arrange(CHANNEL, YEAR, desc(PROD_REV)) %>%
#     mutate(ITEMNAME_CHI = factor(ITEMNAME_CHI, levels = unique(ITEMNAME_CHI)))
#   ytd$ITEMNAME_CHI <- droplevels(ytd$ITEMNAME_CHI)
#   # ytd$YOY_LABEL <- droplevels(ytd$YOY_LABEL)
# 
#   # Add traces
#   fig <- ytd %>% plot_ly() %>%
#     add_trace(x = ~PROD_REV, y = ~reorder(ITEMNAME_CHI, PROD_REV), type = "bar", orientation = 'h',
...
#     ) %>%
#     add_trace(x = ~PROD_REV_CUM_PCT, y = ~reorder(ITEMNAME_CHI, PROD_REV_CUM_PCT), type = "scatter", mode = "lines+markers", name = "Accumulate %", xaxis = "x2",
...
#     add_trace(x = c(0, ~max(PROD_REV)*1.1), y = c(head(ytd$ITEMNAME_CHI[ytd$PROD_REV_CUM_PCT >= 0.8], 1), head(ytd$ITEMNAME_CHI[ytd$PROD_REV_CUM_PCT >= 0.8], 1)),
#               type = 'scatter', mode = 'lines', line = list(color = 'blue', dash = "dash"), name = "At least 80%", hoverinfo='skip') %>%
#     add_text(mode = "text", x = ~PROD_REV/2, y = ~reorder(ITEMNAME_CHI, PROD_REV), symbol = ~YOY_LABEL,
...
temp_data <- rbind(
  raw_data_6 %>% 
    group_by(Brand, SubBrand, ITEMNAME_CHI, YEAR, MONTH) %>%
    summarise(PROD_REV = sum(PROD_REV)
              ,PROD_QTY = sum(PROD_QTY)) %>% 
    mutate(CHANNEL = "Total: MNG + WTC")
  ,
  raw_data_6 %>% 
    group_by(CHANNEL, Brand, SubBrand, ITEMNAME_CHI, YEAR, MONTH) %>% 
    summarise(PROD_REV = sum(PROD_REV)
              ,PROD_QTY = sum(PROD_QTY))
) %>% 
  mutate(CHANNEL = factor(CHANNEL, levels = c("Total: MNG + WTC", "MNG", "WTC"))
...
yoy_and_mom <- lapply(list("PROD_REV", "PROD_QTY"), function(measure_value){
  ...
  temp_data <- temp_data %>% 
    # YOY
    group_by(CHANNEL, Brand, SubBrand, ITEMNAME_CHI, MONTH) %>%
    mutate(LAST_YEAR = ifelse((YEAR - lag(YEAR, 1)) > 1, NA, lag(YEAR, 1))
           ,measure_value_LAST_YEAR = ifelse((YEAR - lag(YEAR, 1)) > 1, NA, lag(get(measure_value), 1))
    ) %>% 
    mutate(YOY = get(measure_value) - measure_value_LAST_YEAR
    ...
    # MOM
    group_by(CHANNEL, Brand, SubBrand, ITEMNAME_CHI) %>%
    mutate(measure_value_LAST_MONTH =  lag(get(measure_value), 1)
           ,MOM = get(measure_value) - lag(get(measure_value), 1)
           ,MOM_PCT = get(measure_value)/lag(get(measure_value), 1)-1
    )
...
ytd <- temp_data %>%
  filter(MONTH <= month(max(raw_data_6$DATE))) %>% 
  group_by(CHANNEL, Brand, SubBrand, ITEMNAME_CHI, YEAR) %>% 
  summarise(PROD_REV = sum(PROD_REV)
            ,PROD_QTY = sum(PROD_QTY)
            ,PROD_REV_LAST_YEAR = sum(PROD_REV_LAST_YEAR)
            ,PROD_QTY_LAST_YEAR = sum(PROD_QTY_LAST_YEAR)
...
plot_YTD_prod_rev_qty <- ggplotly(shared_temp_data %>% ggplot(., aes(x = PROD_QTY, y = PROD_REV)) +
                                    geom_point(aes(color = Brand
                                                   ...
                                                   , shape = as.character(YEAR)
                                                   , alpha = 0.1
                                                   , text = paste0("Product: ", "<i><b>",ITEMNAME_CHI,"</i></b>",
                                                                   "\nProd Rev: ", "<i><b>", label_dollar(accuracy = 1)(PROD_REV),"</i></b>", "\nProd Qty: ", "<i><b>", comma(PROD_QTY, accuracy = 1),"</i></b>",
                                                                   "\n\nYOY:\n",
                                                                   "<i><b>",label_dollar(accuracy = 1)(ytd$PROD_REV_YOY), " (",label_percent(accuracy = 1)(ytd$PROD_REV_YOY_PCT),")", "</i></b>\n",
                                                                   "<i><b>",label_dollar(accuracy = 1)(ytd$PROD_QTY_YOY), " (",label_percent(accuracy = 1)(ytd$PROD_QTY_YOY_PCT),")", "</i></b>\n")
...
#          , aes(x = YEAR, y = PROD_REV, color = ITEMNAME_CHI)) +
#     geom_line(size = 1) +
#     geom_point(size=2) +
...
######################################## Product Revenue Trend ########################################
# Time series Plot
plot_time_series_prod <- function(ytd, temp_data, i, highlight_group_name) {
  
  legend_grp_filter_rank <- ytd %>% ungroup() %>% filter(CHANNEL == i & YEAR == max(YEAR)) %>% arrange(Brand, desc(PROD_REV)) %>% 
    mutate(legend_grp_filter_rank = paste(Brand, "-", ITEMNAME_CHI)) %>% 
    mutate(legend_grp_filter_rank = factor(legend_grp_filter_rank, levels = legend_grp_filter_rank)) %>% 
    select(CHANNEL, Brand, ITEMNAME_CHI, legend_grp_filter_rank)
  
  temp_data <- temp_data %>% ungroup() %>% filter(CHANNEL == i) %>% 
    mutate(DATE = as.Date(fast_strptime(paste0(YEAR,"-",MONTH), "%Y-%m", lt = FALSE))) %>% 
    left_join(legend_grp_filter_rank, by = c("CHANNEL", "Brand", "ITEMNAME_CHI")) %>% 
    arrange(legend_grp_filter_rank)
  
  temp_data <- data.table(temp_data)[is.na(legend_grp_filter_rank), legend_grp_filter_rank := paste(Brand, "-", ITEMNAME_CHI)]
  
  shared_temp_data <- temp_data %>% highlight_key(., ~ITEMNAME_CHI, group = highlight_group_name)
  
  ######################################## Product Revenue Trend ########################################
  plot_time_series_prod <- ggplotly(shared_temp_data %>% 
                                      ggplot(., aes(x = DATE, y = PROD_REV, color = legend_grp_filter_rank, group = legend_grp_filter_rank, text = paste0("Product: ", "<i><b>", legend_grp_filter_rank, "</i></b>", "\nDate: ", "<i><b>", DATE, "</i></b>", "\nProd Rev ", "<i><b>", label_dollar(accuracy = 1)(PROD_REV), "</i></b>"))) +
                                      geom_point(size=2) +
                                      geom_line(size = 1) +
                                      # geom_smooth(method = "loess", formula = y ~ x, na.rm = T, alpha = 0.2, linetype = 5, size = 0.5) +
                                      labs(title = paste0("Product Revenue Trend")
                                      ...
                                      theme_ben() +
                                      theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
                                    # scale_color_manual(drop = T, values = setNames(c("yellow3", "black", "orange", "limegreen", "red3"), levels(temp_data$ITEMNAME_CHI)))
                                    # facet_wrap_paginate(~ CHANNEL, ncol = 1, nrow = 1, page = i) %>% 
                                    , dynamicTicks = TRUE
                                    , tooltip = c("text")
  ) %>%
  ...
  ######################################## MOM: Product Revenue Trend ########################################
  plot_time_series_prod_mom <- ggplotly(shared_temp_data %>% 
                                          ggplot(., aes(x = DATE, y = PROD_REV_MOM_PCT, color = legend_grp_filter_rank, group = legend_grp_filter_rank
                                                        , text = paste0("Product: ", "<i><b>", legend_grp_filter_rank, "</i></b>", "\nDate: ", "<i><b>", DATE, "</i></b>", "\nProd Rev ", "<i><b>", label_dollar(accuracy = 1)(PROD_REV), "</i></b>",
    ...
    #           # , persistent = TRUE
    # ) %>%
    theme_layout(caption = "", caption_margin = 100, y = -0.33) %>%
    layout(
      # legend = list(x = 0.05, y = 0.95),
      xaxis = list(autorange = TRUE, hoverformat = "%Y-%m-%d"),
      yaxis = list(autorange = TRUE, tickformat = ".0%")
  ...
  adj_legendgroup <- function(pp){
    # Adjust legend name in ggplotly 
    for (i in 1:length(pp$x$data)) {
      # pp$x$data[[i]]$name <- str_extract(pp$x$data[[i]]$name, "[^\\(][^,]*")
      pp$x$data[[i]]$legendgroup <- strsplit(pp$x$data[[i]]$legendgroup, " - ")[[1]][1]
    }
    return(pp)
  }
  
  plot_combine <- subplot(plot_time_series_prod
                          , plotly::style(plot_time_series_prod_mom, showlegend = FALSE) %>%
                            layout(annotations = list(
                              list(x = 0.5 , y = 1.03, text = "MOM", showarrow = F, xref='paper', yref='paper'))
                            )
                          , nrows = 2, heights = c(0.7, 0.3), titleY = TRUE, shareX = TRUE, margin = 0.03) %>% 
    # config(displaylogo = FALSE, modeBarButtons = list(list("toImage"), list("hoverClosestCartesian"), list("hoverCompareCartesian"))) %>%
    highlight(., on = "plotly_click", off = "plotly_doubleclick", selected = attrs_selected(showlegend = FALSE), selectize = TRUE
              , persistent = TRUE
    ) %>%
    adj_legendgroup
  
  # plot_time_series_prod <- plot_time_series_prod %>% adj_legendgroup
  # 
  # plot_time_series_prod_mom <- plot_time_series_prod_mom %>% adj_legendgroup
  
  # return(list(plot_time_series_prod, plot_time_series_prod_mom))
  return(plot_combine)
}
```

### Total: MNG + WTC
```{r fig.asp = 1, fig.width = 15, out.width = "100%"}
plot_time_series_prod(ytd, temp_data, "Total: MNG + WTC", "ITEMNAME_CHI")
```
### MNG
```{r fig.asp = 1, fig.width = 15, out.width = "100%"}
# ```{r fig.asp = 0.8, fig.width = 15, out.width = "100%"}
# ```{r fig.asp = 1.3, fig.width = 9}
plot_time_series_prod(ytd, temp_data, "MNG", "ITEMNAME_CHI ")
```
### WTC
```{r fig.asp = 1, fig.width = 15, out.width = "100%", results='asis'}
plot_time_series_prod(ytd, temp_data, "WTC", "ITEMNAME_CHI  ")
```
...
temp_data <- raw_data_6 %>%
  mutate(ITEMNAME_CHI = trimws(ITEMNAME_CHI)) %>%  #After revised raw_data_6, remove code line
  distinct(CHANNEL, DATE, ITEMNAME_CHI, .keep_all = T) %>%   #After revised raw_data_6, remove code line
  select(CHANNEL, DATE, ITEMNAME_CHI, PROD_REV) %>%      #After revised raw_data_6, remove code line
  group_by(DATE, ITEMNAME_CHI) %>%
  summarise(PROD_REV = sum(PROD_REV)) %>%
  pivot_wider(names_from = ITEMNAME_CHI, values_from = PROD_REV, values_fill = 0) %>%
...
corr_df <- corr_df %>% filter((abs(CORR) >= 0.5) & P_VALUE < 0.05) %>%
  ...
  ungroup() %>%
  left_join(select(heatmaply_side_colors, ITEMNAME_CHI, RANK), by = c("Var1" = "ITEMNAME_CHI")) %>%
  arrange(RANK) %>%
...
upper_tri_corr <- upper_tri_corr %>% as.data.frame(row.names = rownames(upper_tri_corr)) %>%
  mutate(ITEMNAME_CHI = row.names(.)) %>%
  left_join(select(heatmaply_side_colors, Brand, ITEMNAME_CHI, PROD_REV), by = "ITEMNAME_CHI") %>%
  tibble::column_to_rownames("ITEMNAME_CHI")
